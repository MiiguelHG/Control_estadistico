...
import { FrequencyData, HistogramaService } from '../../services/histograma/histograma.service';
...
export class HistogramaComponent implements OnInit {
  ...
  histogramaService = inject(HistogramaService);
  ...
  private calculateFrequencyTable(): void {
    this.frequencyTable = this.histogramaService.getTable();
    this.total = this.histogramaService.getTotal();
  }

  private prepareChartOptions(): void {
    this.chartOptions = this.histogramaService.getHistogramaFrecuencias();
  }

  private prepareChartOptionsRelative(): void {
    this.chartOptionsRelative = this.histogramaService.getHistogramaRelativas();
  }

  private prepareChartOptionsAcumulado(): void {
    this.chartOptionsAcumulado = this.histogramaService.getHistogramaAcumuladas();
  }

  private prepareChartOptionsRelativeAcumulado(): void {
    this.chartOptionsRelativeAcumulado = this.histogramaService.getHistogramaRelativasAcumuladas();
  }

  public updateData(data: number[][]): void {
    this.histogramaService.setData(data);
    this.loadInitialData();
